...
public abstract class LootPoolSingletonContainer extends LootPoolEntryContainer {
    ...
    protected static <T extends LootPoolSingletonContainer> Products.P4<RecordCodecBuilder.Mu<T>, Integer, Integer, List<LootItemCondition>, List<LootItemFunction>> singletonFields(RecordCodecBuilder.Instance<T> instance) {
        return instance.group(ExtraCodecs.strictOptionalField(Codec.INT, "weight", 1).forGetter((entry) -> {
            return entry.weight;
        }), ExtraCodecs.strictOptionalField(Codec.INT, "quality", 0).forGetter((entry) -> {
            return entry.quality;
        })).and(commonFields(instance).t1()).and(ExtraCodecs.strictOptionalField(LootItemFunctions.CODEC.listOf(), "functions", List.of()).forGetter((entry) -> {
