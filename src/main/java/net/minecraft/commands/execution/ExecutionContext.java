...
public class ExecutionContext<T> implements AutoCloseable {
    ...
    public static <T extends ExecutionCommandSource<T>> void queueInitialFunctionCall(ExecutionContext<T> context, InstantiatedFunction<T> procedure, T source, CommandResultCallback returnValueConsumer) {
        context.queueNext(new CommandQueueEntry<>(createTopFrame(context, returnValueConsumer), (new CallFunction<>(procedure, source.callback(), false)).bind(source)));
    }

    public static <T extends ExecutionCommandSource<T>> void queueInitialCommandExecution(ExecutionContext<T> context, String command, ContextChain<T> contextChain, T source, CommandResultCallback returnValueConsumer) {
    ...
    public Frame.FrameControl frameControlForDepth(int depth) {
        return () -> {
            this.discardAtDepthOrHigher(depth);
        };
    }
