...
public class BuildContexts<T extends ExecutionCommandSource<T>> {
    @VisibleForTesting
    public static final DynamicCommandExceptionType ERROR_FORK_LIMIT_REACHED = new DynamicCommandExceptionType((count) -> {
        return Component.translatableEscape("command.forkLimit", count);
    });
                ...
                for(int i = context.forkLimit(); contextChain.getStage() != Stage.EXECUTE; contextChain = contextChain.nextStage()) {
                    ...
                    if (redirectModifier != null) {
                        ...
                        for(T executionCommandSource : list) {
                            try {
                                Collection<T> collection = ContextChain.runModifier(commandContext, executionCommandSource, (contextx, successful, returnValue) -> {
                                }, bl);
        ...
        if (list.isEmpty()) {
            ...
            if (command instanceof CustomCommandExecutor) {
                ...
                if (chainModifiers.isReturn()) {
                    ...
                    executionCommandSource3 = executionCommandSource3.withCallback(CommandResultCallback.chain(executionCommandSource3.callback(), frame.returnValueConsumer()));
                ...
                ContinuationTask.schedule(context, frame, list, (framex, source) -> {
                    return new CommandQueueEntry<>(framex, executeCommand.bind(source));
                });
            }
