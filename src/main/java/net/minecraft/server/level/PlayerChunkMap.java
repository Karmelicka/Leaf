...
public class PlayerChunkMap extends IChunkLoader implements PlayerChunk.d {
    ...
    private CompletableFuture<Either<List<IChunkAccess>, PlayerChunk.Failure>> getChunkRangeFuture(PlayerChunk playerchunk, int i, IntFunction<ChunkStatus> intfunction) {
        if (i == 0) {
            ChunkStatus chunkstatus = (ChunkStatus) intfunction.apply(0);

            return playerchunk.getOrScheduleFuture(chunkstatus, this).thenApply((either) -> {
            ...
            for (int l = -i; l <= i; ++l) {
                for (int i1 = -i; i1 <= i; ++i1) {
                    ...
                    CompletableFuture<Either<IChunkAccess, PlayerChunk.Failure>> completablefuture = playerchunk1.getOrScheduleFuture(chunkstatus1, this);
            ...
            while (iterator.hasNext()) {
                ...
                playerchunk2.addSaveDependency("getChunkRangeFuture " + chunkcoordintpair + " " + i, completablefuture2);
            }
    ...
    public CompletableFuture<Either<Chunk, PlayerChunk.Failure>> prepareEntityTickingChunk(PlayerChunk playerchunk) {
    ...
    private CompletableFuture<Either<IChunkAccess, PlayerChunk.Failure>> scheduleChunkGeneration(PlayerChunk playerchunk, ChunkStatus chunkstatus) {
        ChunkCoordIntPair chunkcoordintpair = playerchunk.getPos();
        CompletableFuture<Either<List<IChunkAccess>, PlayerChunk.Failure>> completablefuture = this.getChunkRangeFuture(playerchunk, chunkstatus.getRange(), (i) -> {
    ...
    private CompletableFuture<Either<IChunkAccess, PlayerChunk.Failure>> protoChunkToFullChunk(PlayerChunk playerchunk) {
        ...
        return completablefuture.thenApplyAsync((either) -> {
            ChunkStatus chunkstatus = ChunkLevel.generationStatus(playerchunk.getTicketLevel());
    ...
    public CompletableFuture<Either<Chunk, PlayerChunk.Failure>> prepareTickingChunk(PlayerChunk playerchunk) {
        CompletableFuture<Either<List<IChunkAccess>, PlayerChunk.Failure>> completablefuture = this.getChunkRangeFuture(playerchunk, 1, (i) -> {
            return ChunkStatus.FULL;
        });
    ...
    public CompletableFuture<Either<Chunk, PlayerChunk.Failure>> prepareAccessibleChunk(PlayerChunk playerchunk) {
        return this.getChunkRangeFuture(playerchunk, 1, ChunkStatus::getStatusAroundFullChunk).thenApplyAsync((either) -> {
            return either.mapLeft((list) -> {
                return (Chunk) list.get(list.size() / 2);
            });
        }, (runnable) -> {
